...
class TrainingParameters:
    def __init__(self, options=None):
        ...
        self.LOW_ALPHA = 0.25 #For Focal Loss
        self.HIGH_ALPHA = 0.75
        self.MID_ALPHA = 0.5
...
class NzBirdData:
    N_FOLDS = 10
    USE_SECONDARY = True
    RARE_THRESHOLD = 10 # Classes with less samples than this will not be allowed in validation dataset, and will be up-sampled to 10
    SPATIAL_LIMITS = None #Filter the dataset by lat and long. For example: {'WEST':0, 'EAST':10, 'NORTH': -20, 'SOUTH':-30}
    MAX_PER_CLASS = 30000   #Cap the maximum number of samples allowed in any particular class
    MAX_PER_CLASS_VAL = 200  #Cap the max for the val classes so that the val score isn't dominated by the common samples
    EXCLUDED_CLASSES = []
    LOW_ALPHA_CLASSES = ['morepo2'  'nezbel1' 'gryger1' 'silver3' 'tomtit1' 'eurbla' 'tui1' 'nezkak1'], #common classes
    HIGH_ALPHA_CLASSES = ['spocra2', 'easros1', 'spocra1', 'redjun1', 'takahe3', 'codpet1', 'chukar', 'caster1', 'parpet1', 
                                   ...
                                   'mallar3', 'baicra4', 'houspa', 'blkswa', 'coopet', 'swahar1', 'calqua', 'blfter1', 'piesti1'], #rare classes
...
class BCEFocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        # If loss_alphas list is provided, use it
        if self.loss_alphas is not None:
            # targets is assumed to be of shape [batch_size, num_classes]
            # alpha values are broadcasted based on class indices
...
def save_models(paths, classes):
    ...
    for ckpt_path in tqdm(checkpoints):
        model = get_model(ckpt_path, cfg, audio, paths, classes)
...
def get_loss_alphas(classes, data, cfg):
    low_indices = [classes.index(x) for x in data.LOW_ALPHA_CLASSES if x in classes]
    high_indices = [classes.index(x) for x in data.HIGH_ALPHA_CLASSES if x in classes]
...
def run_training(dl_train, dl_val, data, cfg, audio, checkpoint_dir):
    ...
    loss_alphas = get_loss_alphas(classes, data, cfg)
    audio_model = BirdSoundModel(cfg, classes, audio, paths, loss_alphas=None)
...
def load_training_data(paths, data):
    ...
    in_df = in_df[~in_df['primary_label'].isin(data.EXCLUDED_CLASSES)]
...
def encode_data(train_df, val_df, use_secondaries, unique_birds):
    if use_secondaries:
...
#These names are a bit silly.  Let's use train_cfg, data_cfg, audio_cfg, paths
cfg = TrainingParameters(options=use_case)
data = NzBirdData()
audio = DefaultAudio()
...
in_df, unique_birds = load_training_data(paths, data)
in_df = filter_by_location(in_df, limits=data.SPATIAL_LIMITS)
in_df = limit_max_per_class(in_df, data.MAX_PER_CLASS)
common_df, rare_df = split_classes_by_size(in_df, data.RARE_THRESHOLD)
rare_df = duplicate_rare_rows(rare_df, data.RARE_THRESHOLD)
train_df, val_df = train_val_split(common_df, rare_df, n_folds=data.N_FOLDS, max_per_class_val=data.MAX_PER_CLASS_VAL)
train_df_0, val_df = encode_data(train_df, val_df, data.USE_SECONDARY, unique_birds)
...
if (__name__ == '__main__') and (cfg.TRAIN):
    metrics = run_training(dl_train, dl_val, data, cfg, audio, paths.chkpt_dir)  #, df_updater
    extract_results(metrics, paths)
    last_path = save_models(paths, unique_birds)
    final_model = BirdSoundModel(cfg, unique_birds, audio, paths)  # Now checking it loads OK
